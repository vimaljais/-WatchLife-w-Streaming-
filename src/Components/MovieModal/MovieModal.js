...
import Ratings from "../ModalRatings/ModalRatings";
...
const MovieModal = (props) => {
  ...
  const [playerAvail, setPlayerAvail] = useState(false);
    ...
    const fetchImdbIDandPoster = async () => {
      try {
        const res = await fetch(
          `https://arcane-sierra-37425.herokuapp.com/tmdb-data?id=${props.tmdb_id}`,
          {
  ...
  useEffect(() => {
    ...
    const fetchIMDBData = async () => {
      try {
        const res = await fetch(
          `https://arcane-sierra-37425.herokuapp.com/imdb-data/?id=${tmdbData.imdb_id}`,
          {
            signal: abortController.signal,
          }
        );
  ...
  useEffect(() => {
    if (typeof imdb === "string") {
      fetch(`https://arcane-sierra-37425.herokuapp.com/getPlayer/?imdb=${imdb}`)
        .then((response) => response.json())
        .then((response) => {
          if (response.player_url) {
            setPlayerAvail(true);
            setPlayerLink(response.player_url);
          }
        })
                ...
                <Grid item xs={12} lg={4} md={4}>
                  ...
                  <Typography variant="subtitle1" className="valueText">
                    {loading ? (
                      ...
                      imdbData.Director && imdbData.Director
                    )}
                  </Typography>
                </Grid>
              </Grid>
